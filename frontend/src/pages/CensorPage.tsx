...
// ============================================
// DATETIME FORMATTING FUNCTIONS (No Timezone Conversion)
// ============================================
// ใช้ regex parse โดยตรง ไม่ใช้ new Date() เพราะจะแปลง timezone อัตโนมัติ
// ซึ่งจะทำให้เวลาผิดถ้าเบราว์เซอร์อยู่คนละ timezone กับ server
...
const formatDateTime = (timestamp: string | null | undefined): string => {
  if (!timestamp) return '-';
  try {
    // Regex to extract date/time components directly (ignores timezone)
...
/**
 * Format time only (HH:MM:SS) for display - no timezone conversion
 */
const formatTimeOnly = (timestamp: string | null | undefined): string => {
  if (!timestamp) return '-';
  try {
    const match = timestamp.match(/[T\s](\d{2}):(\d{2})(?::(\d{2}))?/i);
    if (match) {
...
/**
 * Format date only for display - no timezone conversion
 */
const formatDateOnly = (timestamp: string | null | undefined): string => {
  if (!timestamp) return '-';
  try {
...
const parseTimestampNoTimezoneConversion = (timestamp: string | null | undefined): Date | null => {
  if (!timestamp) return null;
  const match = timestamp.match(/^\s*(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2})(?::(\d{2}))?/);
...
const getFreshnessInfo = (timestamp: string | null | undefined): { label: string; color: 'success' | 'warning' | 'error' | 'default' } | null => {
  const parsed = parseTimestampNoTimezoneConversion(timestamp);
  if (!parsed) return null;

  const diffMs = Date.now() - parsed.getTime();
  if (diffMs < 0) {
    return { label: 'เวลาเครื่องไม่ตรง', color: 'default' };
  }
  ...
  if (totalMinutes <= 30) {
    return { label: 'ข้อมูลสด', color: 'success' };
  }

  if (totalMinutes <= 120) {
    return {
      label: `ล่าช้า ${hours > 0 ? `${hours}ชม.` : ''}${minutes}น.`,
      color: 'warning'
    };
  }

  return {
    label: `ข้อมูลเก่า ${hours}ชม.${minutes}น.`,
    color: 'error'
  };
};
...
const MetricCardComponent: React.FC<MetricCardProps> = ({ metric, onClick }) => {
  ...
  // ใช้ formatDateTime แทน toBangkokTime
  const latestTimestamp = useMemo(() => {
    if (!metric.latest_time) return null;
    return formatDateTime(metric.latest_time);
  }, [metric.latest_time]);

  const freshnessInfo = useMemo(() => getFreshnessInfo(metric.latest_time), [metric.latest_time]);
  ...
  // ใช้ formatDateOnly แทน toBangkokDate
  const dateRangeDisplay = useMemo(() => {
    if (!metric.first_seen || !metric.last_seen) return null;
    return `${formatDateOnly(metric.first_seen)} - ${formatDateOnly(metric.last_seen)}`;
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.25, flexWrap: 'wrap' }}>
                ...
                {freshnessInfo && (
                  <Chip
                    ...
                    variant="outlined"
                    label={freshnessInfo.label}
                    sx={{ height: 20, '& .MuiChip-label': { px: 0.75, fontSize: '0.67rem', fontWeight: 600 } }}
                  />
                )}
              </Box>
            )}
...
const CensorPage: React.FC = () => {
  ...
  const { 
    ...
    queryKey: ['censor-enhanced-metrics', selectedSite, selectedEquipment, period, appliedStartDate, appliedEndDate],
    queryFn: () => {
      const params: any = {};
  ...
  const { data: detailData, isLoading: detailLoading } = useQuery({
    queryKey: ['censor-metric-details', selectedDetailMetric, selectedSite, selectedEquipment, period, appliedStartDate, appliedEndDate],
    queryFn: () => {
      const params: any = {
  ...
  }, [period, draftEndDate, draftStartDate]);
        ...
        <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={3}>
          <Box>
            ...
            <Typography variant="h6" color="textSecondary">
              ระบบวิเคราะห์เซ็นเซอร์แบบเรียลไทม์ - เลือกไซต์และอุปกรณ์เพื่อดูข้อมูล
            </Typography>
            <Typography variant="caption" color="primary" sx={{ mt: 1, display: 'block' }}>
              ✨ หน้านี้ใช้การแสดงเวลาแบบใหม่ที่แก้ไขปัญหา timezone แล้ว
            </Typography>
          </Box>
