...
// Modern Enhanced Rack Component (Fixed)
...
// Enhanced Airflow Particles Component (Fixed)
const AirflowParticles = ({ 
  ...
  useEffect(() => {
    ...
    const safeIntensity = typeof intensity === 'number' && intensity > 0 ? Math.max(0.1, Math.min(intensity, 3)) : 1;
    
    // Initialize particles
    const initialParticles: AirFlowParticle[] = Array.from({ length: Math.floor(30 * safeIntensity) }, (_, i) => ({
      id: i.toString(),
    ...
    const animate = () => {
      setParticles(prev => prev.map(particle => ({
        ...
        opacity: Math.max(0.1, 0.3 + Math.sin(Date.now() * 0.01 + parseInt(particle.id)) * 0.3)
      })));
    };
...
// Main Data Center View with Enhanced Features (Fixed)
const EnhancedDataCenterView = ({ 
  ...
  // Calculate optimal stage dimensions (with safety checks)
  const stageDimensions = useMemo(() => {
    ...
    if (racks && Array.isArray(racks)) {
      racks.forEach(rack => {
        if (rack && rack.position && typeof rack.position.x === 'number' && typeof rack.position.y === 'number') {
    ...
    // Fallback to default values if no valid positions found
    if (minX === Infinity) {
      minX = 0;
      maxX = 1200;
      minY = 0;
          ...
          <Box sx={{ mt: 2, pt: 2, borderTop: 1, borderColor: 'divider' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
              <Chip
                size="small"
                icon={<Storage />}
                label={`ตู้ทั้งหมด: ${racks ? racks.length : 0}`}
                color="info"
                variant="outlined"
              />
              <Chip
                ...
                label={`ใช้งาน: ${racks ? racks.filter(r => r.isActive).length : 0}`}
                color="success"
                variant="outlined"
              />
              <Chip
