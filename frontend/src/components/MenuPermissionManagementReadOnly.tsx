/**
 * Menu Permission Management Component (Read-Only Version)
 * แสดงเมนูและสิทธิ์การเข้าถึงเมนูสำหรับแต่ละบทบาท (ดูอย่างเดียว)
 * รวมจัดการเมนูและจัดการสิทธิ์ในหน้าเดียวกัน
 */
  ...
  const createMenuMutation = useMutation({
    mutationFn: async (data: typeof menuFormData) => {
      ...
      if (!newMenu || !newMenu.id) {
        throw new Error('เมนูถูกสร้างแล้ว แต่ไม่ได้รับ ID กลับมา');
      }
      ...
      for (const role of roles) {
        ...
        if (canView) {
          console.log(`Setting permission for role ${role.name} (${role.id}) on menu ${newMenu.id}`);
              ...
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ bgcolor: 'grey.50' }}>
                    ...
                    <TableCell align="center" sx={{ fontWeight: 'bold', width: '120px' }}>จัดการ</TableCell>
