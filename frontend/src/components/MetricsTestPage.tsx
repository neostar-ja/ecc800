...
const MetricsTestPage: React.FC = () => {
  // State management
  ...
  // API configuration - using test endpoints
  const API_BASE = '/ecc800/api/metrics/test';
  ...
  const loadSystemStatus = async () => {
    try {
      const response = await axios.get<TestApiResponse<SystemStatus>>(`${API_BASE}/status`);
  ...
  const loadSites = async () => {
    try {
      setLoading(true);
      const response = await axios.get<TestApiResponse<SiteSummary[]>>(`${API_BASE}/sites`);
      ...
      if (response.data.status === 'success') {
        ...
        if (response.data.data.length > 0 && !selectedSite) {
          setSelectedSite(response.data.data[0].site_code);
        }
      } else {
  ...
  const loadDevices = async (siteCode: string) => {
    try {
      const response = await axios.get<TestApiResponse<Device[]>>(`${API_BASE}/devices`, {
      ...
      if (response.data.status === 'success') {
        ...
        if (response.data.data.length > 0 && !selectedDevice) {
          setSelectedDevice(response.data.data[0].device_code);
        }
      } else {
        setError(`API Error: ${response.data.message}`);
      }
  ...
  const loadMetrics = async (siteCode: string, deviceCode: string) => {
    try {
      ...
      const response = await axios.get<TestApiResponse<MetricsResponse>>(`${API_BASE}/metrics`, { params });
  ...
  const loadTimeSeries = async (siteCode: string, deviceCode: string, metrics: string[]) => {
    try {
      const response = await axios.get<TestApiResponse<TimeSeriesResponse>>(`${API_BASE}/timeseries`, {
        params: {
          site_code: siteCode,
          device_code: deviceCode,
  ...
  return (
    <Box p={3}>
      <Typography variant="h4" gutterBottom>
        🧪 ECC800 Metrics Dashboard - Test Version
      </Typography>
      
      <Alert severity="info" sx={{ mb: 3 }}>
        หน้าทดสอบ API ใหม่สำหรับระบบเมทริก - ใช้ database views และไม่ต้องล็อกอิน
      </Alert>
