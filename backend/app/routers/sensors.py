...
async def get_latest_sensor_data(
    ...
    try:
        ...
        # Prevent "could not resize shared memory segment" error in constrained docker environments
        await db.execute(text("SET max_parallel_workers_per_gather = 0"))
        result = await db.execute(sensor_query, {"site": site.upper() if site else None})
        await db.execute(text("RESET max_parallel_workers_per_gather"))
